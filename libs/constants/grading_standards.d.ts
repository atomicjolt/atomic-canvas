export namespace createNewGradingStandardAccounts {
    const type: string;
    const method: string;
    const key: string;
    const required: string[];
}
export namespace createNewGradingStandardCourses {
    const type_1: string;
    export { type_1 as type };
    const method_1: string;
    export { method_1 as method };
    const key_1: string;
    export { key_1 as key };
    const required_1: string[];
    export { required_1 as required };
}
export namespace listGradingStandardsAvailableInContextCourses {
    const type_2: string;
    export { type_2 as type };
    const method_2: string;
    export { method_2 as method };
    const key_2: string;
    export { key_2 as key };
    const required_2: string[];
    export { required_2 as required };
}
export namespace listGradingStandardsAvailableInContextAccounts {
    const type_3: string;
    export { type_3 as type };
    const method_3: string;
    export { method_3 as method };
    const key_3: string;
    export { key_3 as key };
    const required_3: string[];
    export { required_3 as required };
}
export namespace getSingleGradingStandardInContextCourses {
    const type_4: string;
    export { type_4 as type };
    const method_4: string;
    export { method_4 as method };
    const key_4: string;
    export { key_4 as key };
    const required_4: string[];
    export { required_4 as required };
}
export namespace getSingleGradingStandardInContextAccounts {
    const type_5: string;
    export { type_5 as type };
    const method_5: string;
    export { method_5 as method };
    const key_5: string;
    export { key_5 as key };
    const required_5: string[];
    export { required_5 as required };
}
